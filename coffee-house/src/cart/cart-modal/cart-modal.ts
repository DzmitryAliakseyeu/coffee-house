...
export default async function createModalCart() {
  ...
  try {
    ...
    let response = await confirmOrderRequest(order)
                ...
                if(response){
                   modal.remove();
                   let productContainer = (document.querySelector('.products-cart-container') as HTMLElement)
                  productContainer.innerHTML = '';
                   ...
                   succesfullMessage.classList.add('heading-3');
                    ...
                    succesfullMessage.textContent = "Thank you for your order! Our manager will contact you shortly."

                    const totalCartPrice = document.querySelector('.order-total-info-text') as HTMLElement;
                    let totalPrice = 0;
